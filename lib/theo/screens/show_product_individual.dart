...
class _ProductDetailPageState extends State<ProductDetailPage> {
  ...
  Future<void> fetchRatings() async {
    try {
      ...
      final response = await request.get('http://127.0.0.1:8000/ratings/${widget.id}/');
      final List data = jsonDecode(response);

      setState(() {
        _ratings = data.map((item) => Rating.fromJson(item)).toList();
        // _isLoading = false;
      });
    } catch (error) {
      setState(() {
        // _errorMessage = 'Failed to load ratings: $error';
        // _isLoading = false;
      });
    }
  }
            ...
            child: Column(
              ...
              children: [
                ...
                _isLoading
                    ...
                    : _ratings.isEmpty
                        ? const Text("No ratings available.") // Message if no ratings
                        : SizedBox(
                            height: 300, // Adjust height as needed
                            child: RatingList(ratings: _ratings), // Display ratings
                          ),
              ],
            ),
          ),
